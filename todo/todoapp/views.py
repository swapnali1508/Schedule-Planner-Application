...
def user_register(request):
    ...
    if request.method=="POST":
        ...
        if un=='' or p=='' or cp=='':
            ...
            content['errmsg']="Password must be at least 8 character in length"
        else:
            try:
...
def user_logout(request):
    logout(request) #destroye session
...
def datefilter(request):
    ...
    # select * from todo_task where enddate>="2023-06-12" and enddate<="2023-06-15";
    return render(request,'todoapp/dashboard.html',content)
...
def datesort(request,dv):
    ...
    if dv=='0':
        col='-enddate'   #descending order
    else:
        col='enddate'     #assending order
...
def sendpendingemail(t):                #it is normal function
    # print("In pending email section:")
    # print(t)
    for x in t:
        if x.status==0:
            d=x.enddate.day
            ...
            if diff==1:
                ...
                sender='swapnalimane1508@gmail.com'
                send_mail(
                subject,
                msg,
                sender,
